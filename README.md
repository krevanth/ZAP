...
| Reset                                              | Purely synchronous reset scheme. Relaxed placement of the fully synchronous (on both, rise and fall of reset) reset signal source is allowed. This allows the pulse on this port to be wide enough but randomly phase shifted allowing a dual rank synchronizer to be used (instead of a reset synchronizer). |
