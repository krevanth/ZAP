...
# Rule to verilate.
obj/ts/$(TC)/Vzrisc_test: $(CPU_FILES) $(TB_FILES) $(SCRIPT_FILES) src/ts/$(TC)/Config.cfg obj/ts/$(TC)/$(TC).bin
ifdef SEED
...
runlint:
	verilator --assert --lint-only -sv -error-limit 1 -Wall -Wpedantic -Wwarn-lint -Wwarn-style -Wwarn-MULTIDRIVEN     \
        -Wwarn-IMPERFECTSCH --report-unoptflat --clk i_clk --top-module zrisc_top src/rtl/*.sv -Isrc/rtl/           \
...
runsim: dirs obj/ts/$(TC)/Vzrisc_test
ifdef TC
	$(info ******************************)
...
ifdef SEED 
ifdef XTERM
	cd obj/ts/$(TC) && xterm -e "./Vzrisc_test $(TC).bin $(TC) $(SEED) || read" &
else
	cd obj/ts/$(TC) && ./Vzrisc_test $(TC).bin $(TC) $(SEED) 
endif
else
ifdef XTERM
	cd obj/ts/$(TC) && xterm -e "./Vzrisc_test $(TC).bin $(TC) || read" &
else
	cd obj/ts/$(TC) && ./Vzrisc_test $(TC).bin $(TC)
endif
endif
	echo "Generated waveform file 'obj/ts/$(TC)/zrisc.vcd'"
else
